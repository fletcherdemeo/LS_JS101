...
// Data Structures:
// Use a dictionary to keep track of elements

// Algorithm:
// 1. Create a dictionary
//    - For number of switches
//      - Create dictionary item including switch number and status
//        i.e , {'1': false}
// 2. For number of switches (increment at switch count)
//    - Flick switch (on to off, off to on)
// 3. Return the dictionary keys filtered by whether or not they are on

function lightsOn(switches) {
  let switchObj = {}
  for (let switchInd = 1; switchInd <= switches; switchInd += 1) {
    switchObj[switchInd] = false;
  }

  for (let switchInd = 1; switchInd <= switches; switchInd += 1) {
    for (let ind = switchInd; ind <= switches; ind += switchInd) {
      switchObj[ind] = !switchObj[ind];
    }
  }

  return Object.keys(switchObj).filter(switchInd => switchObj[switchInd]);
}
